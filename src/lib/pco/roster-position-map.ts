...
let rosterPositionMapPathCache: string | undefined;

export function clearRosterPositionMapCache(): void {
  rosterPositionMapCache = undefined;
  rosterPositionMapPathCache = undefined;
}
...
export function loadRosterPositionMapFromDisk(mapPath?: string): Record<string, string> {
  ...
  if (rosterPositionMapCache !== undefined && rosterPositionMapPathCache === resolved) {
    return rosterPositionMapCache;
  }

  try {
    const raw = readFileSync(resolved, "utf8");
    rosterPositionMapCache = parseRosterMapJson(raw);
    rosterPositionMapPathCache = resolved;
    return rosterPositionMapCache;
  } catch {
    rosterPositionMapCache = {};
    rosterPositionMapPathCache = resolved;
    return rosterPositionMapCache;
  }
}

/** Cached loader used by plan-team and APIs. */
export function loadRosterPositionMap(): Record<string, string> {
  return loadRosterPositionMapFromDisk();
}
